...
# Parâmetros
#aparentemente 10 de L e 3 pontos é o limite
L = 10
pontos = 3
evitar = []
...
inicio = (-15, 115)
fim = (115, -15)
todos_caminhos = list(nx.all_simple_paths(grafo, source=inicio, target=fim))

print(f"{len(todos_caminhos)} caminhos encontrados.")

MAX_CAMINHOS = 100
caminhos_plotar = todos_caminhos[:MAX_CAMINHOS]
...
for caminho in caminhos_plotar:
    for i in range(len(caminho) - 1):
        x_vals = [caminho[i][0], caminho[i+1][0]]
        y_vals = [caminho[i][1], caminho[i+1][1]]
        plt.plot(x_vals, y_vals, color='green', linewidth=1, alpha=0.3, zorder=4)
