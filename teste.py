...
def ponto_dentro_do_quadrado(px, py, quadrado):
    ...
    return xmin < px < xmax and ymin < py < ymax  # Exclusivo para evitar bordas

def segmento_dentro_do_quadrado(p1, p2, quadrado):
    return ponto_dentro_do_quadrado(p1[0], p1[1], quadrado) and ponto_dentro_do_quadrado(p2[0], p2[1], quadrado)
...
for i in range(0, len(sla1), 4):
    x_vals = sla1[i:i+4] + [sla1[i]]
    y_vals = sla2[i:i+4] + [sla2[i]]
    plt.plot(x_vals, y_vals, color='blue')

for i in range(0, len(sla1_final)):
    for j in range(i + 1, len(sla1_final)):
        ...
        for quadrado in quadrados:
            ...
            if segmento_dentro_do_quadrado(p1, p2, quadrado):
                cruza_algum_lado = True
                break
